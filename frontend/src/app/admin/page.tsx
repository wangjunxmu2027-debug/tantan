...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"companies" | "links">("companies");
  ...
  const createLink = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setCreateForm({ company_name: "", interviewer_name: "", expires_hours: 0, max_uses: 0 });
        fetchLinks();
      }
    } catch (error) {
  ...
  // 导出为 CSV
  const exportToCSV = () => {
    const headers = ["公司名称", "访谈者", "链接", "访问次数", "完成次数", "完成率", "创建时间", "过期时间"];
